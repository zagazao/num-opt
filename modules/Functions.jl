...
export f_square,g_square,f_logreg,g_logreg,f_rosenbrock,g_rosenbrock,g_rosenbrock2
...
function g_logreg(X,y,θ,λ)
    return function(θ)
        ...
        for i in 1:size(data,2)
            ...
            x = data[i:i,1:size(data,2)]'
            exponential = exp(-labels[i]*θ'*x)
            tmp = (-labels[i]*x[i] * exponential)/(1+exponential) + λ*θ[i]
            append!( array, tmp )
        end
        return array
    end
